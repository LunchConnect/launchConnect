...
const StartDashboardNav = ({ onMenuClick }: { onMenuClick: () => void }) => {
  ...
  return (
    <nav className="w-full lg:w-[calc(100%-256px)] ml-auto flex justify-between items-center py-4 px-6 border-b bg-white fixed top-0 right-0 z-50 md:px-6">
      ...
      {/* Welcome Message */}
      <h1 className="text-xl font-semibold text-gray-800">Hey, {fullName}</h1>
    {/* Profile Section */}
<div className="relative" ref={dropdownRef}>
  ...
  {isDropdownOpen && (
    <div className="absolute right-0 mt-2 w-56 bg-white shadow-lg rounded-md border p-2">
      <button
        onClick={() => {
          router.push("/startup_founder_dashboard/profilemanagement");
          scrollToTop();
        }}
...
export default StartDashboardNav;
